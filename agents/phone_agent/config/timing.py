...
class ScreenshotTimingConfig:
    """Configuration for screenshot operation timing and retry settings."""

    # Screenshot operation settings
    ...
    def __post_init__(self):
        """Load values from environment variables if present."""
        self.timeout = float(
            os.getenv("PHONE_AGENT_SCREENSHOT_TIMEOUT", self.timeout)
        )
        self.retry_count = int(
            os.getenv("PHONE_AGENT_SCREENSHOT_RETRY_COUNT", self.retry_count)
        )
        self.retry_delay = float(
            os.getenv("PHONE_AGENT_SCREENSHOT_RETRY_DELAY", self.retry_delay)
        )
        self.pull_timeout = float(
            os.getenv("PHONE_AGENT_SCREENSHOT_PULL_TIMEOUT", self.pull_timeout)
        )
